...
function renewToken(refreshToken, _deviceId) {
  ...
  if (horasRestantes >= 168) // 168 horas = 7 dias (sempre renova o refreshToken)
...
function updateSessionStatus(deviceId) {
  const sheet = sheetSession(env().SHEETNAME_REFRESH);
  const row = findTextInColumn(sheet, 'E', deviceId);
  if (!row.success)
    return {success: false, message: 'não encontrado'};
  sheet.getRange(row.result, 6).setValue('desativado');
  return {success: true, message: `status do dispositivo ${deviceId} foi desativado`}
}
...
function findTextInColumn(sheet, column, textToFind, bGetRowPosition = true) {
